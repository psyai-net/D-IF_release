...
class Evaluator:
    ...
    def set_mesh(self, result_dict, export_dir):
        ...
        self.verts_gt[:, 1] *= -1
        # self.verts_smpl = projection(self.verts_smpl, self.calib)
        # self.verts_smpl[:, 1] *= -1
        ...
        self.smpl_mesh = self.render.VF2Mesh(self.verts_smpl, self.faces_smpl)
        # import trimesh
        # import os
        # print(export_dir.split('/')[-1])
        # prrecon_obj = trimesh.Trimesh(self.verts_pr.detach().cpu(), self.faces_pr.detach().cpu(), process=False, maintains_order=True)
        # prrecon_obj.export('/data/yangxueting/'+'prrecon_obj.obj')
        # gtrecon_obj = trimesh.Trimesh(self.verts_gt.detach().cpu(), self.faces_gt.detach().cpu(), process=False, maintains_order=True)
        # gtrecon_obj.export('/data/yangxueting/'+'gtrecon_obj.obj')
        # smplrecon_obj = trimesh.Trimesh(self.verts_smpl.detach().cpu(), self.faces_smpl.detach().cpu(), process=False, maintains_order=True)
        # smplrecon_obj.export('/data/yangxueting/'+'smplrecon_obj.obj')
    ...
    def calculate_normal_consist(self, normal_path):
        ...
        ################### draw by yxt ########################
        # yxterror = (1- torch.sigmoid((src_normal_arr - tgt_normal_arr)**2)) * 2.0
        # # print(yxterror,'???', yxterror.shape)
        # error_img = Image.fromarray(
        #     (
        #         yxterror.permute(1, 2, 0).detach().cpu().numpy() * (255.0,255.0,255.0)
        #     ).astype(np.uint8)
        # )
        # error_img = ImageEnhance.Contrast(error_img)
        # error_img_path = '.'+normal_path.split('.')[-2] + '_errormap.png'
        # error_img.enhance(40).save(error_img_path)
        # # error_img.save('./hii.jpg')
        # stop
        ############################################################
    ...
    def calculate_chamfer_p2s(self, num_samples=1000):
        ...
        smpl_points = Pointclouds(sample_points_from_meshes(self.smpl_mesh, num_samples))
        ...
        # New for pred and smpl
        smpl_p2s_dist = point_mesh_distance(self.src_mesh, smpl_points) * 100.0
        smpl_chamfer_dist = (point_mesh_distance(self.smpl_mesh, src_points) * 100.0 + smpl_p2s_dist) * 0.5
        # distance between smpl and mesh 来衡量衣服宽松不宽松
        dd_p2s_dist = point_mesh_distance(self.smpl_mesh, tgt_points) * 100.0
        dd_chamfer_dist = (point_mesh_distance(self.tgt_mesh, smpl_points) * 100.0 + dd_p2s_dist) * 0.5
        return chamfer_dist, p2s_dist, smpl_p2s_dist, smpl_chamfer_dist, dd_p2s_dist, dd_chamfer_dist
