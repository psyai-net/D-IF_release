...
def rescale_smpl(fitted_path, scale=100, translate=(0, 0, 0)):
    ...
    # scaled = fitted_path[:-4]+ '_scaled97.obj'
    # print('fitted_path',fitted_path, scaled)
    # fitted_body.apply_transform(resize_matrix).export(scaled)
    
    # scale = 100
    # fitted_body = trimesh.load(fitted_path, process=False, maintain_order=True, skip_materials=True)
    # resize_matrix = trimesh.transformations.scale_and_translate(scale=(scale), translate=translate)

    # scaled = fitted_path[:-4]+ '_scaled100.obj'
    # print('fitted_path',fitted_path, scaled)
    # fitted_body.apply_transform(resize_matrix).export(scaled)
    # stop
...
class SMPLX():
    def __init__(self):
        ...
        self.current_dir = '/data/yangxueting/ICON_orl/data/smpl_related'
