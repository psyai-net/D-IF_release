...
class PIFuDataset():
    def __init__(self, cfg, split='train', vis=False):
        ...
        self.split = split
        self.root = '/data/yangxueting/ICON_orl/data'
    ...
    def knnocc(self, inputX, data, labels, k):
    ...
    def get_sampling_geo(self, data_dict, is_valid=False, is_sdf=False):
        ...
        # offset_1 = np.random.normal(scale=1, size=(16000, 1))
        # offset_3 = np.random.normal(scale=3, size=(4000, 1))
        # offset_5 = np.random.normal(scale=5, size=(4000, 1))
        # offset = np.concatenate((offset_0, offset_1, offset_3, offset_5),axis=0)

        samples_surface += mesh.vert_normals[samples_surface_ids, :] * offset
        # print(mesh.vert_normals[samples_surface_ids, :])
        ...
        plt.savefig('/data/yangxueting/draw_pifu_v3_1/mysample/lm-{}.png'.format(time.time()))
        ...
        knn_labels = []
        # add by yxt 统计计算 gt occ值 
        start = time.time()
        iddd = 0
        i_sample_num = 10   # 50-0.12 30-0.1 20-0.07 10-0.06 5-0.03 3-0.02
        err = 0
        for i_sample in samples:
            iddd += 1
            # print(iddd)
            # 1cm3小球半径为0.62035，球内sample50点
            # occupancy
            # i_offset = (np.random.rand(i_sample_num,1) - 0.5) * 1.2407
            # # 随机空间方向偏移offset，方向是单位为1的向量
            # i_rand_normal = np.random.rand(i_sample_num,3) - 0.5
            # i_scale = (i_rand_normal[:,0]**2 + i_rand_normal[:,1]**2 + i_rand_normal[:,2]**2)**0.5
            # i_rand_normal[:,0] /= i_scale
            # i_rand_normal[:,1] /= i_scale
            # i_rand_normal[:,2] /= i_scale
            # i_occu_sample = i_rand_normal * i_offset + i_sample
            # i_inside = mesh.contains(i_occu_sample)
            # i_occ_gt = np.sum(i_inside != 0) / i_sample_num
            i_lable = self.knnocc(i_sample, samples, labels_01occu, i_sample_num)
            # print(i_lable, labels_01occu[iddd-1])
            if (labels_01occu[iddd-1] == 0 and i_lable >= 0.5) or (labels_01occu[iddd-1] != 0 and i_lable < 0.5):
                err += 1
            knn_labels.append(i_lable)
        print(err / 8000)
        labels = np.array(knn_labels)
