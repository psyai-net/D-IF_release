...
class PIFuDataset():
    ...
    def get_sampling_geo(self, data_dict, is_valid=False, is_sdf=False):
        ...
        # vert_ids = np.arange(mesh.verts.shape[0]) # (6890, 3)
        ...
        samples = mesh.verts
        # Sampling offsets are random noise with constant scale (15cm - 20cm)
        offset = np.random.normal(scale=self.opt.sigma_geo, size=(mesh.verts.shape[0], 1))
        samples += mesh.vert_normals * offset * 0.000001
        
        print(mesh.verts.shape,'???')
        labels = np.ones(samples.shape[0])-0.5
        # draw sampling points
        ax = plt.subplot(projection = '3d')  # 创建一个三维的绘图工程
        ax.set_title('3d_image_show')  # 设置本图名称
        ax.scatter(samples[:,0], samples[:,1], samples[:,2], c = 'r')   # 绘制数据点 c: 'r'红色，'y'黄色，等颜色
        ...
        # sdf_labels = torch.from_numpy(sdf_labels).float()
        # sdf_labels = torch.sigmoid(sdf_labels)
        # sdf_labels = 1.0/(1+ torch.exp(-10000*sdf_labels))
        # errcnt = 0
        # for i in range(len(sdf_labels)):
        #     if ((labels[i] == 0) and (sdf_labels[i] > 0.5)) or ((labels[i] != 0) and (sdf_labels[i] < 0.5)):
        #         errcnt += 1
        # print(errcnt / 8000,'????????????????')
