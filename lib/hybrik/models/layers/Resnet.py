...
class Bottleneck(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, norm_layer=nn.BatchNorm2d, dcn=None
    ):
...
class ResNet(nn.Module):
    ...
    def make_layer(self, block, planes, blocks, stride=1, dcn=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, norm_layer=self._norm_layer, dcn=dcn)
        )
