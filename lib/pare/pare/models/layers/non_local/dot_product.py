...
if __name__ == '__main__':
    ...
    net = NONLocalBlock1D(in_channels=256, inter_channels=None, sub_sample=False, bn_layer=True)
    out = net(img)
    print(out.size())

    img = torch.zeros(2, 256, 56, 56)
    net = NONLocalBlock2D(in_channels=256, inter_channels=None, sub_sample=False, bn_layer=True)
    out = net(img)
    print(out.size())
