...
for sex in ['male', 'female', 'neutral']:
    for age in ['kid', 'adult']:
        ...
        input_mesh = trimesh.Trimesh(smpl.verts, smpl.faces, maintain_order=True, process=False)
        ...
        if True:
            ...
            posed_shape_added = tet_smpl_vertices_added - \
                smpl.trans.reshape([1, 3])
            posed_shape_added_h = np.hstack(
                (posed_shape_added, np.ones([posed_shape_added.shape[0], 1]))
            )
            for ti in range(len(T)):
                T[ti] = np.linalg.inv(T[ti])
            rest_shape_added = np.matmul(T, posed_shape_added_h.reshape([-1, 4,
                                                                         1])).reshape([-1,
                                                                                       4])[:, :3]

            tetra_smpl_structure = np.load(f'./data/tetgen_{sex}_{age}_structure.npy')
            np.savez_compressed(
                f'./data/tetra_{sex}_{age}_smpl.npz',
            ...
            np.savetxt(
                f'./data/tetrahedrons_{sex}_{age}.txt',
                np.int32(tetra_smpl_structure + 1),
                fmt='%d'
            )

            smpl = TetraSMPLModel(
                f'./smpl/SMPL_{sex.upper()}.pkl', f'./data/tetra_{sex}_{age}_smpl.npz', age
            )
